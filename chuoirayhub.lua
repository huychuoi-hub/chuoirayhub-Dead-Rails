...
-- Tabs
local Tabs = {
    FarT = Window:AddTab({ Title = "Farm", Icon = "wheat" }),
    Ma = Window:AddTab({ Title = "Main", Icon = "home" }),
    Ite = Window:AddTab({ Title = "Items", Icon = "package" }),
    TpT = Window:AddTab({ Title = "Teleport", Icon = "map-pin" }),
...
do
    local baseUrl = "https://raw.githubusercontent.com/RENBex6969/SkullHUB-deadrails/refs/heads/main/"
    local Config = loadstring(game:HttpGet(baseUrl .. "Config.lua"))()
    local Utilities = loadstring(game:HttpGet(baseUrl .. "Util.lua"))()
    ...
    AfBToggle:OnChanged(function(Value)
        if Value then
            ...
            AutoBondsURL = baseUrl .. "AutoBonds.lua"
    ...
    AfBToggle1:OnChanged(function(Value)
        if Value then
            AutoBondsURl = baseUrl .. "AutoBonds2.lua"
            loadstring(game:HttpGet(AutoBondsURl))()
        end
    end)
    ...
    AfBToggle12:OnChanged(function(Value)
        if Value then
            AutoBondsUrL = baseUrl .. "AutoBonds3.lua"
            loadstring(game:HttpGet('AutoBondsUrL'))() 
        end
    end)
    ...
    Tabs.Ma:AddButton({
        ...
        Callback = function()
            ...
            Icon.Image = "rbxassetid://95183678717613" -- Replace with actual image asset ID
    ...
    -- UI Setup
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "SkullHub_NPC_Lock_UI"
    ...
    icon.Image = "rbxassetid://95183678717613" -- Replace with your image ID
    icon.Parent = button
    icon.ZIndex = 1001
                ...
                if tool then
                    ...
                    for _, npc in ipairs(npcs) do
                        if npc.hum and npc.hum.Health > 0 then
                            ...
                            if not success then
                                warn("[chuoiray HUB] Failed to fire shot: " .. tostring(err))
                            end
                            if AutoReloadEnabled then
                                ...
                                if not success then
                                    warn("[chuoiray HUB] Failed to reload: " .. tostring(err))
                                end
                            end
                        end
                    end
...
do
    if game.PlaceId == 70876832253163 then
        local MoneyToggle = Tabs.Ite:AddToggle("MoneyToggle", {Title = "Auto Collect Money Bag", Default = false})
        local Mstatus = false
        ...
        MoneyToggle:OnChanged(function(Value)
            ...
            if Mstatus then
                ...
                cleanupConnections() -- Stop all active loops
                restoreMoneybagPrompts() -- Restore hold duration for Moneybags
            end
        end)        

        local PsoBToggle = Tabs.Ite:AddToggle("PsoBToggle", {Title = "Auto Pickup Snake Oil & Bandage(s)", Default = false})
        ...
        PsoBToggle:OnChanged(function(Value)
            ...
            if autoPickupEnabled then
                ...
                pickupLoop = RunService1.Heartbeat:Connect(function()
        ...
        -- Define the toggle
        local AutoBToggle = Tabs.Ite:AddToggle("AutoBToggle", {Title = "Auto Pickup Bond(s)", Default = false })
        ...
        -- Toggle logic
        AutoBToggle:OnChanged(function(Value)
            if Value then
                -- Start periodic checks when toggle is enabled
                if remote then
                    connection = RunService.Heartbeat:Connect(function()
                        -- Run every 0.5 second (adjust as needed)
                        if tick() % 0.5 < 0.05 then
                            pcall(collectBonds)
                        end
                    end)
                else
                    warn("Cannot start bond collection: No valid remote found.")
                    Toggle:SetValue(false) -- Disable toggle if no remote
                end
            else
                -- Disconnect when toggle is disabled
                if connection then
                    connection:Disconnect()
                    connection = nil
                end
        ...
        local AmmoToggle = Tabs.Ite:AddToggle("AmmoToggle", {
            Title = "Auto Pickup Ammo(s)",
            Default = false
        })
        ...
        local function startAutoCollect()
            ...
            ammoConnection = runtimeItems.ChildAdded:Connect(function(child)
                if autoCollectEnabled and table.find(ammoTypes, child.Name) then
                    task.wait(0.1) -- Small delay to ensure ammo is registered
                    remotePromise1:FireServer(child)
                end
            end)
        end

        AmmoToggle:OnChanged(function(Value)
        ...
        Tabs.Ite:AddSection("↳ Brings (Buggy)")

        Tabs.Ite:AddButton({
            Title = "Collect Snake Oil",
            Callback = function()
                local player = game.Players.LocalPlayer
                local character = player.Character
                if not character then
                    return warn("[chuoiray HUB] Character not found!")
                end
                local hrp = character:FindFirstChild("HumanoidRootPart")
                if not hrp then
                    return warn("[chuoiray HUB] HumanoidRootPart not found!")
                end
                local runtimeItems = workspace:FindFirstChild("RuntimeItems")
                if not runtimeItems then
                    return warn("[chuoiray HUB] RuntimeItems folder not found!")
                end

                -- Move all Bandages to player
                for _, item in ipairs(runtimeItems:GetChildren()) do
                    if item:IsA("Model") and item.Name == "Snake Oil" then
                ...
                local sixthItem = runtimeItems:GetChildren()[6]
                local fifthItem = RuntimeItems:GetChildren()[5]
                if fifthItem and fifthItem:IsA("Model") and fifthItem.Name == "Snake Oil" then
                    local primaryPart = fifthItem.PrimaryPart or fifthItem:FindFirstChildOfClass("Part")
                    if primaryPart then
                        primaryPart.CFrame = hrp.CFrame
                    end
                elseif sixthItem and sixthItem:IsA("Model") and sixthItem.Name == "Snake Oil" then
                    local primaryPart = sixthItem.PrimaryPart or sixthItem:FindFirstChildOfClass("Part")
                    if primaryPart then
                        primaryPart.CFrame = hrp.CFrame
                    end
                end
        ...
        Tabs.Ite:AddButton({
            ...
            Callback = function()
                ...
                local sixthItem = runtimeItems:GetChildren()[6]
                local fifthItem = RuntimeItems:GetChildren()[5]
                if sixthItem and sixthItem:IsA("Model") and sixthItem.Name == "Bandage" then
                    local primaryPart = sixthItem.PrimaryPart or sixthItem:FindFirstChildOfClass("Part")
                    if primaryPart then
                        primaryPart.CFrame = hrp.CFrame
                    end
                elseif fifthItem and fifthItem:IsA("Model") and fifthItem.Name == "Bandage" then
                    local primaryPart = fifthItem.PrimaryPart or fifthItem:FindFirstChildOfClass("Part")
                    if primaryPart then
                        primaryPart.CFrame = hrp.CFrame
                    end
        ...
        Tabs.Ite:AddButton({
            ...
            Callback = function()
                ...
                local hrp = character:FindFirstChild("[chuoiray HUB] HumanoidRootPart")
                if not hrp then
                    return warn("[chuoiray HUB] HumanoidRootPart not found!")
                end
                ...
                -- Iterate through RuntimeItems
                for _, item in ipairs(runtimeItems:GetChildren()) do
                    if item:IsA("Model") then
                        local itemName = item.Name:lower()

                        -- Check if the Model's name contains "Ammo"
        ...
        Tabs.Ite:AddParagraph({
            Title = "Note",
            Content = "Join the main game to load full this tab!"
        })
    end
end
...
do
    if game.PlaceId == 116495829188952 then
        ...
        CountdownGui.Parent = game:GetService("CoreGui") -- or PlayerGui if needed
        ...
        local countdownTime = 10 * 60 -- 10 minutes
        local running = true
        
        task.spawn(function()
            while countdownTime > 0 and running do
        ...
        Tabs.TpT:AddSection("↳ Other Locations")

        Tabs.TpT:AddButton({
            Title = "TP To Vampire Castle",
            Callback = function()
                ...
                GoToVampurl = baseUrl .. "GotoVamp.lua"
                loadstring(game:HttpGet(GoToVampurl))()
        ...
        local function teleportToLocation(locationName)
            ...
            if Character and Character:FindFirstChild("HumanoidRootPart") then
                ...
                if fseat then
                    ...
                    Fluent:Notify({
                        Title = "chuoiray HUB",
                        Content = "Base Teleport",
                        SubContent = "No seat found near location: " .. locationName,
                        Duration = 5
                    })
                end
            ...
            Callback = function()
                if selectedLocation and teleportLocations[selectedLocation] then
                    ...
                    Fluent:Notify({
                        Title = "chuoiray HUB",
                        Content = "Base Teleport",
                        SubContent = "No valid base selected for teleportation.",
                        Duration = 5
                    })
                end
            end
...
InterfaceManager:SetFolder("chuoirayl HUB")
SaveManager:SetFolder("chuoiray HUB/Dead Rails")
...
Fluent:Notify({ Title = "chuoiray HUB", Content = "Dead Rails script loaded successfully!", Duration = 5 })
...
Button.Image = "rbxassetid://95183678717613" -- Replace with your custom image ID
